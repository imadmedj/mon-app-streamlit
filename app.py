# ══════════════════════════════════════════════════════════════════════════════
# TÉLÉCHARGEMENT AUTOMATIQUE — doit être AVANT tout autre import lourd
# ══════════════════════════════════════════════════════════════════════════════
...
# Afficher un écran de chargement pendant le téléchargement
_dl_placeholder = st.empty()
with _dl_placeholder.container():
    st.markdown("## ⏳ Initialisation du système HSV...")
    st.info("Téléchargement des modèles et données depuis Google Drive. Merci de patienter (1 à 5 min au 1er démarrage).")

from download_data import download_all
_ok = download_all()

if not _ok:
    st.error("❌ Certains fichiers n'ont pas pu être téléchargés. Vérifiez les logs ci-dessus.")
    st.stop()

_dl_placeholder.empty()   # Efface l'écran de chargement

# ══════════════════════════════════════════════════════════════════════════════
# IMPORTS NORMAUX (après téléchargement)
# ══════════════════════════════════════════════════════════════════════════════
"""
╔══════════════════════════════════════════════════════════════════════════════╗
...
with st.sidebar:
    ...
    if page == T("realtime_pred"):
        ...
        if pred_model_page == T("pred_m1_label"):
            ...
            st.markdown(f"<div style='background:{bg_c};border:1px solid {border_c};border-radius:8px;padding:6px 10px;font-size:.72rem;color:{active_badge_color};margin-top:4px;'>{active_badge_text}</div>", unsafe_allow_html=True)
        else:
...
if page == T("realtime_pred"):
    ...
    if pred_wants_m2 and not is_m2:
        st.markdown("""<div class="incompat-banner"><div class="ib-icon">🚫</div><div class="ib-title">Incompatibilité de modèle</div><div class="ib-body">Vous avez sélectionné <strong>Prédiction M2 — ERA5 + CMEMS</strong> mais le modèle de données actif est <strong>🔵 M1 — ERA5 seul</strong>.</div><div class="ib-step">👉 <strong>Solution :</strong> Dans la sidebar, sélectionnez <strong>🟣 M2 — ERA5 + CMEMS (1999–2023)</strong></div></div>""", unsafe_allow_html=True)
        st.stop()

    elif pred_wants_m1 and is_m2:
        st.markdown("""<div class="incompat-banner"><div class="ib-icon">🚫</div><div class="ib-title">Incompatibilité de modèle</div><div class="ib-body">Vous avez sélectionné <strong>Prédiction M1 — ERA5 seul</strong> mais le modèle de données actif est <strong>🟣 M2 — ERA5 + CMEMS</strong>.</div><div class="ib-step">👉 <strong>Solution :</strong> Dans la sidebar, sélectionnez <strong>🔵 M1 — ERA5 seul (1985–2023)</strong></div></div>""", unsafe_allow_html=True)
        st.stop()

    elif pred_wants_m2 and is_m2:
        page_header("#8b5cf6","🟣","Prédiction Temps Réel — M2","ERA5 + CMEMS → LSTM enrichi → HSV")
        st.markdown("""<div class="wip-banner"><div class="wb-icon">🚧</div><div class="wb-title">Module M2 en cours de développement</div><div class="wb-body">Le pipeline de prédiction <strong>Modèle 2</strong> (ERA5 + CMEMS) est en cours de développement. Disponibilité estimée : Prochaine version du dashboard.</div></div>""", unsafe_allow_html=True)
        st.stop()

    # ── PIPELINE M1 COMPLET ────────────────────────────────────────────
    page_header("#8b5cf6","🔮","Prédiction Temps Réel — M1","ERA5 (contexte 72h) + Open-Meteo Marine → LSTM → HSV → Alerte")

    WINDOW   = 72
    HORIZONS = [1, 6, 12, 24]
    FEATURES = ["MESURE","wind_speed","mwp","mwd_sin","mwd_cos","hour_sin","hour_cos","month_sin","month_cos","day_sin","day_cos","year_sin","year_cos","x_norm","y_norm"]
    DANGER_THRESHOLDS = {"vert":(0.0,0.5,"Mer calme","🟢"),"jaune":(0.5,1.0,"Mer agitée","🟡"),"orange":(1.0,1.5,"Risque modéré","🟠"),"rouge":(1.5,9.0,"DANGER","🔴")}

    import glob, zipfile, traceback, joblib, cdsapi, xarray as xr, tensorflow as tf
    ...
    def asymmetric_huber_loss(delta=0.5, underestimate_penalty=3.0):
        ...
        loss.__name__="asymmetric_huber"; return loss
    ...
    CUSTOM_OBJECTS={"asymmetric_huber":asymmetric_huber,"asymmetric_huber_loss":asymmetric_huber_loss(),"loss":asymmetric_huber_loss()}
    ...
    def load_finetuned_lstm(plage_name):
        base_dir=os.path.dirname(LSTM_PATH); safe_name=plage_name.replace(" ","_")
        ...
        if not os.path.exists(model_path):
            ...
            if not matched: return None,False
    ...
    def _run_inference(X_tensor,plage_name):
        ...
        if used_local and local_model is not None:
            model_used=local_model; model_info=f"Fine-tuné — {plage_name}"
        else:
            model_used=load_global_lstm(); used_local=False; model_info="Global (fallback)"
    ...
    def _model_badge(used_local,plage):
        if used_local: return f"<span style='background:#7c3aed;color:white;padding:3px 10px;border-radius:12px;font-size:0.8em;'>🎯 Modèle fine-tuné — {plage}</span>"
        return "<span style='background:#2563eb;color:white;padding:3px 10px;border-radius:12px;font-size:0.8em;'>🌐 Modèle global (fallback)</span>"

    def _open_era5_file(path,lat,lon):
        def _sel_point(ds):
            if "latitude" in ds.coords and "longitude" in ds.coords:
                ds=ds.sel(latitude=lat,longitude=lon,method="nearest")
            return ds.to_dataframe().reset_index()
        if zipfile.is_zipfile(path):
            tmp_extract=path+"_extract"; os.makedirs(tmp_extract,exist_ok=True)
            with zipfile.ZipFile(path,"r") as zf: zf.extractall(tmp_extract)
            nc_files=glob.glob(os.path.join(tmp_extract,"*.nc"))
            if not nc_files: raise RuntimeError("Aucun .nc dans le ZIP ERA5")
            dfs=[]
            for nc in nc_files:
                ds=xr.open_dataset(nc,engine="netcdf4"); dfs.append(_sel_point(ds)); ds.close()
        ...
        for engine in ["netcdf4","h5netcdf","scipy"]:
            try:
                ds=xr.open_dataset(path,engine=engine); df=_sel_point(ds); ds.close(); return df
            except: continue
        raise RuntimeError(f"Impossible d'ouvrir ERA5 : {path}")

    def _fetch_era5_window(c,lat,lon,date_start,date_end,tmp_dir):
        days_needed=[]; d=date_start.date()
        while d<=date_end.date(): days_needed.append(d); d+=timedelta(days=1)
        month_groups=defaultdict(list)
        for day in days_needed: month_groups[(day.year,day.month)].append(day)
        frames=[]
        for (yr,mo),days in month_groups.items():
            dl_path=os.path.join(tmp_dir,f"era5_{yr}_{mo:02d}.zip")
            c.retrieve("reanalysis-era5-single-levels",{"product_type":"reanalysis","variable":["significant_height_of_combined_wind_waves_and_swell","10m_u_component_of_wind","10m_v_component_of_wind","mean_wave_period","mean_wave_direction"],"year":[str(yr)],"month":[f"{mo:02d}"],"day":[f"{x.day:02d}" for x in days],"time":[f"{h:02d}:00" for h in range(24)],"area":[lat+0.5,lon-0.5,lat-0.5,lon+0.5],"format":"netcdf"},dl_path)
            df_tmp=_open_era5_file(dl_path,lat,lon); frames.append(df_tmp)
        ...
        df=df.sort_values("DATETIME"); df=df[(df["DATETIME"]>=date_start)&(df["DATETIME"]<=date_end)].copy()
        return df
    ...
    def _fetch_openmeteo_forecast(lat,lon,date_start,date_end):
        ...
        params=f"latitude={lat:.4f}&longitude={lon:.4f}&hourly=wave_height,wave_period,wave_direction,wind_speed_10m,wind_direction_10m&start_date={start_str}&end_date={end_str}&timezone=UTC"
        url=f"https://marine-api.open-meteo.com/v1/marine?{params}"
        try:
            with urllib.request.urlopen(url,timeout=30) as resp: data=_json.loads(resp.read().decode())
        except Exception as e: raise RuntimeError(f"Erreur Open-Meteo Marine API : {e}")
        if "hourly" not in data: raise RuntimeError(f"Réponse Open-Meteo invalide.")
        h=data["hourly"]
        df=pd.DataFrame({"DATETIME":pd.to_datetime(h["time"]),"MESURE":h.get("wave_height",[np.nan]*len(h["time"])),"mwp":h.get("wave_period",[np.nan]*len(h["time"])),"mwd":h.get("wave_direction",[np.nan]*len(h["time"])),"wind_speed_raw":h.get("wind_speed_10m",[np.nan]*len(h["time"])),"wind_dir_raw":h.get("wind_direction_10m",[np.nan]*len(h["time"]))})
        ws=df["wind_speed_raw"].fillna(0).values; wd=df["wind_dir_raw"].fillna(0).values
    ...
    def _build_context_window(era5_df,forecast_df,target_dt,window_h=72):
        ...
        merged=pd.concat(frames,ignore_index=True).sort_values("DATETIME").drop_duplicates("DATETIME")
    ...
    def _build_features(df,lat,lon):
        ...
        df["MESURE"]=np.clip(df["MESURE"],0,20); df["mwp"]=np.clip(df["mwp"],0,25)
        df["wind_speed"]=np.clip(np.sqrt(df["u10"]**2+df["v10"]**2),0,60)
        df["mwd_sin"]=np.sin(np.deg2rad(df["mwd"])); df["mwd_cos"]=np.cos(np.deg2rad(df["mwd"]))
        h=df["DATETIME"].dt.hour; m=df["DATETIME"].dt.month; d=df["DATETIME"].dt.day
        ...
        df["day_sin"]=np.sin(2*np.pi*d/31); df["day_cos"]=np.cos(2*np.pi*d/31)
        df["year_sin"]=0.0; df["year_cos"]=0.0
        df["x_norm"]=np.clip((lon+2)/11,0,1); df["y_norm"]=np.clip((lat-36)/2,0,1)
    ...
    def _prepare_window(df,scaler):
        X=df[FEATURES].values.astype(np.float32); X=np.nan_to_num(X,nan=0.0,posinf=0.0,neginf=0.0)
        X=scaler.transform(X); X=np.clip(X,-10,10)
        if len(X)<WINDOW: pad=np.tile(X[0],(WINDOW-len(X),1)); X=np.vstack([pad,X])
        X=X[-WINDOW:]; return X.reshape(1,WINDOW,len(FEATURES))

    def _physical_calibration(preds,df_feat,pred_dt):
        preds=np.array(preds,dtype=float); n=len(df_feat); month=pred_dt.month
        ...
        mwd_coast_factor=max(0.8,1.0+0.15*(-mwd_sin_last)); calibrated=np.empty(len(HORIZONS),dtype=float)
        ...
        for idx,(h_horizon,w_size) in enumerate(zip(HORIZONS,window_sizes)):
            ...
            if month in [11,12,1,2,3]:
                winter_floor=0.8+0.03*h_horizon; raw_pred=max(raw_pred,winter_floor)
                wind_mult=1.0
            ...
            p25=float(np.percentile(sub["MESURE"],25)); t=h_horizon/24.0
            floor=recent_hsv*0.85*(1-t)+max(0.30,p25)*t; raw_pred=max(raw_pred,floor)
    ...
    with col_cfg:
        ...
        if data_ok:
            plages_all=q(f"SELECT DISTINCT NOM_PLAGE,FIRST(X) AS lon,FIRST(Y) AS lat FROM {VIEW} GROUP BY NOM_PLAGE ORDER BY NOM_PLAGE")
            plage_names=plages_all["NOM_PLAGE"].tolist()
        else:
            plages_all=pd.DataFrame(); plage_names=[]
        ...
        st.markdown("---")
        today=datetime.utcnow().date(); era5_cutoff=today-timedelta(days=5)
        max_date=today+timedelta(days=1); default_date=today+timedelta(days=1)
        pred_date=st.date_input("📅 Date cible",value=default_date,min_value=today-timedelta(days=30),max_value=max_date)
        pred_dt=datetime.combine(pred_date,datetime.min.time())
        if pred_date<=era5_cutoff: st.info("📡 Mode ERA5 pur"); use_forecast=False
        elif pred_date<=today: st.warning("🔀 Mode hybride ERA5 + Open-Meteo"); use_forecast=True
        else: st.success(f"🚀 Mode forecast · Open-Meteo Marine\n{pred_date.strftime('%d/%m/%Y')}"); use_forecast=True
        st.markdown("---")
        cds_key=st.text_input("🔑 CDS API KEY (ERA5)",type="password",help="Optionnelle si Open-Meteo suffit.")
        ...
        if has_local: st.success(f"✅ Modèle fine-tuné disponible pour **{selected_plage}**")
        else: st.info("ℹ️ Modèle global utilisé pour cette plage")
    ...
    if run_btn:
        try:
            now_utc=datetime.utcnow(); era5_cutoff=now_utc-timedelta(days=5)
            window_start=pred_dt-timedelta(hours=WINDOW); window_end=pred_dt
            progress=st.progress(0); status=st.empty()
            status.info("⚙️ Chargement du scaler..."); progress.progress(8)
            scaler=load_scaler()
            df_era5=None; df_forecast=None; sources_used=[]
            era5_window_end=min(window_end,era5_cutoff); era5_window_start=era5_window_end-timedelta(hours=WINDOW)
            if era5_window_end>era5_window_start and cds_key.strip():
                try:
                    status.info("🛰️ Téléchargement ERA5..."); progress.progress(20)
                    os.environ["CDSAPI_KEY"]=cds_key.strip(); os.environ["CDSAPI_URL"]="https://cds.climate.copernicus.eu/api"
                    c_era5=cdsapi.Client(quiet=True); tmp_dir=tempfile.mkdtemp()
                    df_era5=_fetch_era5_window(c_era5,lat,lon,era5_window_start,era5_window_end,tmp_dir)
                    if not df_era5.empty: sources_used.append(f"✅ ERA5 ({len(df_era5)} pts)")
                except Exception as e_era5: st.warning(f"⚠️ ERA5 non disponible : {e_era5}"); df_era5=None
            if use_forecast or df_era5 is None:
                try:
                    status.info("🌊 Récupération Open-Meteo Marine..."); progress.progress(35)
                    om_start=window_start; om_end=window_end+timedelta(hours=24)
                ...
                except Exception as e_om: st.warning(f"⚠️ Open-Meteo Marine : {e_om}"); df_forecast=None
            if df_era5 is None and df_forecast is None: st.error("❌ Aucune source de données disponible."); st.stop()
            status.info("🔀 Fusion des sources..."); progress.progress(50)
            ...
            status.info("🔧 Feature engineering..."); progress.progress(62)
            df_feat=_build_features(df_raw,lat,lon)
            status.info("📐 Préparation tenseur..."); progress.progress(72)
            X_tensor=_prepare_window(df_feat,scaler)
            status.info("🧠 Inférence LSTM..."); progress.progress(84)
            ...
            preds=np.nan_to_num(preds,nan=1.5,posinf=5.0,neginf=0.5); preds=np.clip(preds,0.05,9.0)
            ...
            with results_area.container():
                ...
                for i,(h,col) in enumerate(zip(HORIZONS,cols)):
                    hsv=float(preds[i]); color,label,emoji=_danger_level(hsv)
                    delta_str="Référence" if i==0 else f"{hsv-float(preds[0]):+.2f} m"
                    with col:
                ...
                fig=go.Figure()
                fig.add_trace(go.Scatter(x=[f"t+{h}h" for h in HORIZONS],y=preds,mode="lines+markers",line=dict(width=4,color="#8b5cf6"),marker=dict(size=14,color=marker_colors,line=dict(width=2,color="white")),name="HSV (m)"))
                ...
                apply_theme(fig); fig.update_layout(title=f"Prévision HSV — {selected_plage}",yaxis_title="HSV (m)",yaxis=dict(range=[0,max(max(preds)*1.25,2.0)]),height=380,showlegend=False)
                st.plotly_chart(fig,use_container_width=True)
        except Exception as e:
            st.error(f"❌ Erreur pipeline : {e}")
            with st.expander("🐛 Traceback complet"): st.code(traceback.format_exc())
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        st.markdown("""<div class="info-card" style="border-left-color:#0ea5e9;"><div class="title" style="color:#38bdf8;">🔵 Modèle 1 — ERA5 (1985–2023)</div><div class="threshold-row"><span class="threshold-key">Période</span><span class="threshold-val" style="color:#38bdf8;">1985 → 2023</span></div><div class="threshold-row"><span class="threshold-key">Lignes</span><span class="threshold-val">~20 millions</span></div><div class="threshold-row"><span class="threshold-key">Variables</span><span class="threshold-val">MESURE · wind_speed · mwp · mwd</span></div></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class="info-card" style="border-left-color:#8b5cf6;"><div class="title" style="color:#a78bfa;">🟣 Modèle 2 — ERA5 + CMEMS (1999–2023)</div><div class="threshold-row"><span class="threshold-key">Période</span><span class="threshold-val" style="color:#a78bfa;">1999 → 2023</span></div><div class="threshold-row"><span class="threshold-key">Lignes</span><span class="threshold-val">~12 millions</span></div></div>""", unsafe_allow_html=True)
    section("📊",T("global_stats"))
    if data_ok: show_kpis()
    else: st.error("❌ Dataset introuvable.")
    if data_ok:
        st.markdown("---"); section("📅",T("annual_evolution"))
        @st.cache_data(show_spinner=False)
        def _annual(view):
        ...
        if not df_yr.empty:
            ...
            fig.add_hline(y=1.5,line_dash="dash",line_color="#f59e0b",annotation_text="Seuil 1.5 m")
            apply_theme(fig); fig.update_layout(title="Évolution annuelle HSV",xaxis_title="Année",yaxis_title="HSV (m)",height=380)
            st.plotly_chart(fig,use_container_width=True)
        ...
        with tab2:
            ...
            with col_a:
                ...
                if not df_dist.empty:
                    fig=go.Figure(go.Bar(x=df_dist["h"],y=df_dist["n"],marker_color="#0ea5e9"))
            ...
            with col_b:
                ...
                if not df_niv.empty:
                    fig=go.Figure(go.Pie(labels=df_niv["NIVEAU"],values=df_niv["n"],hole=0.55,marker_colors=[DANGER_COLORS.get(n,"#666") for n in df_niv["NIVEAU"]]))
        ...
        with tab3:
            ...
            with col1:
                ...
                if not df_seas.empty:
                    fig=go.Figure(go.Bar(x=df_seas["SEASON"],y=df_seas["avg_hsv"],marker_color=[SEASON_COLORS.get(s,"#0ea5e9") for s in df_seas["SEASON"]]))
            ...
            with col2:
                ...
                if not df_hr.empty:
                    fig=go.Figure(go.Scatter(x=df_hr["HOUR"],y=df_hr["avg_hsv"],mode="lines+markers",fill="tozeroy",line=dict(color="#06b6d4",width=2)))
                    apply_theme(fig); fig.update_layout(title="Cycle diurne",xaxis_title=T("hour"),yaxis_title="HSV (m)",height=340)
                    st.plotly_chart(fig,use_container_width=True)
        with tab4:
            df_pl=q(f"SELECT NOM_PLAGE,NOM_WILAYA,AVG(MESURE) AS avg_hsv,MAX(MESURE) AS max_hsv,COUNT(*) AS n FROM {VIEW} {wh} GROUP BY NOM_PLAGE,NOM_WILAYA ORDER BY avg_hsv DESC LIMIT 30")
            if not df_pl.empty:
                fig=go.Figure(go.Bar(x=df_pl["avg_hsv"],y=df_pl["NOM_PLAGE"],orientation="h",marker_color=df_pl["avg_hsv"].apply(lambda v:"#ef4444" if v>=1.5 else "#f59e0b" if v>=1 else "#10b981")))
                apply_theme(fig); fig.update_layout(title="Top 30 plages — HSV moyenne",xaxis_title="HSV (m)",height=max(400,len(df_pl)*22),yaxis=dict(autorange="reversed",**PLOTLY_THEME["yaxis"]))
                st.plotly_chart(fig,use_container_width=True)
    ...
    st.info(f"Page activités — {activity_page}")

elif page == T("synthesis"):
    page_header("#8b5cf6","📋","Synthèse & Export","Tableaux récapitulatifs")
    wh=W()
    tab1,tab2=st.tabs([T("synth_by_beach"),T("export")])
    with tab1:
        df_synth=q(f"SELECT NOM_PLAGE,NOM_WILAYA,COUNT(*) AS n,ROUND(AVG(MESURE),3) AS avg_hsv,ROUND(MAX(MESURE),2) AS max_hsv,ROUND(STDDEV(MESURE),3) AS std_hsv FROM {VIEW} {wh} GROUP BY NOM_PLAGE,NOM_WILAYA ORDER BY avg_hsv DESC")
    ...
    wh=W()
    df_map=q(f"SELECT NOM_PLAGE,NOM_WILAYA,FIRST(X) AS lon,FIRST(Y) AS lat,ROUND(AVG(MESURE),3) AS avg_hsv,ROUND(MAX(MESURE),2) AS max_hsv FROM {VIEW} {wh} GROUP BY NOM_PLAGE,NOM_WILAYA HAVING lon IS NOT NULL AND lat IS NOT NULL")
    if not df_map.empty:
        fig=px.scatter_mapbox(df_map,lat="lat",lon="lon",color="avg_hsv",size="avg_hsv",hover_name="NOM_PLAGE",color_continuous_scale="Blues",size_max=20,zoom=5,mapbox_style="carto-darkmatter")
